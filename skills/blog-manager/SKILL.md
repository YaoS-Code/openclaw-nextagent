---
name: blog-manager
description: Your Blog 管理 — 创建/发布文章、触发自动抓取、审核草稿、语义搜索
metadata:
  {
    "openclaw": {
      "emoji": "📰",
...
> Blog 是 your-domain.com 的子模块，前端在 `/zh/blog` 和 `/en/blog`。
> 整站架构见 `skills/your-website/SKILL.md`。

API Base: `http://localhost:4001`
Bot Token: 同 my_agent_db_bot_token
...
| insights | YourPlatform 实战 | the operator经验分享、案例 |
...
1. the operator口述或提供要点
2. 用 AI 扩写成中英文 Markdown
3. category=`insights`, author=`Your Name`, is_auto=false
4. 直接 published=true
